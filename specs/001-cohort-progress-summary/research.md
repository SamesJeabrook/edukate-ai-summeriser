...
## Decision: Risk evidence is generated by deterministic rules

- **Decision**: Do not accept source-provided at-risk flags. Generate escalation alerts and aggregate risk evidence from validated progress fields and configured intervention rules.
- **Rationale**: The canonical packet contains progress evidence, while deterministic rules provide reproducible, traceable risk signals without trusting an unverified source label.
- **Alternatives considered**: Accepting source flags was rejected because they are absent from the authoritative input fixture and would make the input contract inconsistent.
